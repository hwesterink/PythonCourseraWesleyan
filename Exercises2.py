...
#%%
def average(numlis):
    """ Function that computes the average of the numbers in a list """
    total = 0
    for number in numlis:
        print(number)
        total += number
    print(len(numlis))
    print("average is", total / len(numlis), "its count is ", len(numlis))
...
def average(nlis):
    ...
    for index in range(lenlist):
        print(nlis[index], end = " ")
        total += nlis[index]
    print()
    print("the average is ", total / lenlist)
...
def simple_poem():
    """ Function that generates a simple four line poem """
...
#%%
def diner_waitress():
    """ Function that assembles your order """
